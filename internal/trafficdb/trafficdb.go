...
func (t *DB) RecordMirror(req *model.MirrorRequest) (int64, error) {
	...
	raw := rawMirror(req)
	res, err := t.db.Exec(`insert into mirror_traffic(agent_id,method,url,headers,body,protocol,timestamp,raw,created_at)
		values(?,?,?,?,?,?,?,?,?)`,
...
func (t *DB) RecordXRayRequest(method, url string, headers map[string][]string, body []byte, status int, response string) (int64, error) {
	...
	res, err := t.db.Exec(`insert into xray_requests(method,url,headers,body,raw,response_status,response,created_at)
		values(?,?,?,?,?,?,?,?)`,
...
func (t *DB) RecordOOB(interaction model.OOBInteraction) (*Match, error) {
	...
	_, err := t.db.Exec(`insert into oob_interactions(protocol,full_id,raw_request,raw_response,remote_address,timestamp,matched_source,matched_id,created_at)
		values(?,?,?,?,?,?,?,?,?)`,
